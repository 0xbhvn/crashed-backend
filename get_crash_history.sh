...
# Script to get crash history from BC.GAME API with better error handling
# Usage: ./get_crash_history.sh [page] [size]
...
# Set URLs
API_URL="https://bc.game/api/game/support/crash/history?page=${PAGE}&size=${SIZE}&_=$(date +%s%3N)"

echo "Fetching crash history (page=$PAGE, size=$SIZE)..."
...
if [ "$HTTP_CODE" -eq 200 ]; then
  ...
  if grep -q "^\s*{" "$TEMP_DIR/response.txt"; then
    # Check if the response contains Cloudflare challenge text
    if grep -q "Just a moment" "$TEMP_DIR/response.txt" || grep -q "challenge" "$TEMP_DIR/response.txt"; then
      echo "Error: Received Cloudflare challenge instead of JSON data"
      cp "$TEMP_DIR/response.txt" "$ERROR_FILE"
      ...
      cp "$TEMP_DIR/response.txt" "$OUTPUT_FILE"
      echo "Successfully retrieved crash history"
      
      # Count items in the response
      ITEMS_COUNT=$(grep -o '"items":\[[^]]*\]' "$OUTPUT_FILE" | grep -o "," | wc -l)
